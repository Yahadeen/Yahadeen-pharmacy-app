/**
 * Notifications. Order progress, prescription decisions and restock alerts —
 * the same rows that arrive as push messages, kept here so nothing is lost when
 * someone swipes a banner away.
 *
 * Tapping a row marks it read and follows its `data` payload to the order or
 * product it is about, which is why the payload shape is checked rather than
 * trusted: it comes from the server and may name a screen that no longer exists.
 */
import { Feather } from '@expo/vector-icons';
import { RADIUS, SPACE, TYPE } from '@pharmago/shared';
import type { AppNotification } from '@pharmago/shared';
...
/** Splits the list into Today / Earlier so a long history stays scannable. */
...
export default function NotificationsScreen() {
  ...
  /** Marks read locally first — the row should dim the instant it is tapped. */
  const markRead = (row: AppNotification) => {
    ...
    void data.markNotificationRead(row.id).catch(() => {
      // Reconciled on the next load; nothing worth interrupting anyone over.
    });
  };
  ...
  const open = (row: AppNotification) => {
    ...
    if (!target) return;
    router.push(
      target.kind === 'order' ? `/(app)/order/${target.id}` : `/(app)/product/${target.id}`,
    );
  };
        ...
        {view.loading ? (
          <>
            {[0, 1, 2, 3].map((i) => (
              <View
                key={i}
                style={[styles.skeletonRow, { borderBottomColor: colors.border }]}
              >
          ...
          <EmptyState
            ...
            message="Order updates, prescription decisions and restock alerts will show up on this screen."
          />
        ) : (
                          ...
                          {!!target && (
                            <>
                              ...
                              <Text style={[TYPE.caption, { color: colors.primary }]}>
                                {target.kind === 'order' ? 'View order' : 'View product'}
                              </Text>
                            </>
                          )}
                        </View>
